...
function SubscriptionContent() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <header>
        <h1 className="text-2xl font-semibold tracking-tight text-base-content">{t('billing.title')}</h1>
        <p className="mt-1 text-base-content/60">{t('billing.subtitle')}</p>
      </header>

      {/* Current status */}
      <section className="mt-8">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-base-content/50">{t('billing.status')}</h2>
        ...
        <dl className="mt-6 max-w-xl space-y-4">
          <div className="flex items-baseline justify-between gap-4 border-b border-hairline pb-3">
            <dt className="text-sm text-base-content/60">{t('billing.expires')}</dt>
            <dd className="font-mono text-sm font-medium tabular-nums text-base-content/70">
          ...
          <div className="flex items-baseline justify-between gap-4 border-b border-base-300 pb-3">
            <dt className="text-sm text-base-content/60">{t('billing.accountPhone')}</dt>
      ...
      <section className="mt-8">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-base-content/50">{t('billing.renewTitle')}</h2>
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <span className="flex items-center gap-2 rounded-full bg-base-200 px-3.5 py-1.5 text-sm text-base-content/80">
            <Shield className="size-4" />
            {t('billing.contactAdmin')}
          </span>
        </div>
        <ul className="mt-5 max-w-xl space-y-2.5 text-sm text-base-content/60">
          {['cost', 'renew', 'features'].map((k) => (
            <li key={k} className="flex items-start gap-2.5">
              <Check className="mt-0.5 size-4 shrink-0 text-base-content" />
              {t(`billing.facts.${k}`)}
            </li>
          ))}
        </ul>
...
function StatusBadge({ status, t }: { status: ServiceStatus; t: (key: string) => string }) {
  const cls =
    status === 'ACTIVE'
      ...
      : status === 'PENDING'
        ? 'border-amber-50 bg-amber-100 text-amber-700'
        : 'border-error-50 bg-error-100 text-error';
  return (
